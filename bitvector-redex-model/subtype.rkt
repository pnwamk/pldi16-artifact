...
(define-metafunction RTR-Base
  ...
  [(theory-of (bv= BV-VAL_1 BV-VAL_2)) BV63]
  [(theory-of (bv<= BV-VAL_1 BV-VAL_2)) BV63]
  [(theory-of (And P ...))
   BV63
...
(define (BV63-prop? p)
  (equal? (term BV63) (term (theory-of ,p))))

(define-judgment-form RTR-Base
  #:mode (provable-in-theory I I I)
  #:contract (provable-in-theory any any any)
  [(where BV63 (theory-of P))
   (where any_assumptions ,(filter BV63-prop? (term (Q ...))))
   (where #t ,(BV63-proves? (term any_assumptions) (term P)))
   -------------------------------- "Th-BV63"
   (provable-in-theory
    BV63
    {Q ...}
...
(module+ test
  ...
  (redex-relation-chk
   ...
   ;; DeMorgans
   [(Env {[x : BV] [y : BV]} {})
    (bv= (bvand (bvnot x) (bvnot y)) (bvnot (bvor x y)))]
   ;; x = y + 1 -> x - 1 = y
   [(Env {[x : BV] [y : BV]} {(bv= x (bvadd (bv63 1) y))})
    (bv= (bvsub x (bv63 1)) y)]
   ;; x > y -> (* 2 x)  (* 2 y)
   [(Env {[x : BV] [y : BV]} {(bv<= y x)
                              (bv<= (bv63 0) y)
                              (bv<= y (bv63 100))
                              (bv<= (bv63 0) x)
                              (bv<= x (bv63 100))})
    (bv<= (bvmul (bv63 2) y) (bvmul (bv63 2) x))]
   ;; DeMorgans
   [(renv {[x : BV] [y : BV]} {})
    (bv= (bvand (bvnot x) (bvnot y)) (bvnot (bvor x y)))]
   ;; x = y + 1 -> x - 1 = y
   [(renv {[x : BV] [y : BV]} {(bv= x (bvadd (bv63 1) y))})
    (bv= (bvsub x (bv63 1)) y)]
   ;; x > y -> (* 2 x)  (* 2 y)
   [(renv {[x : BV] [y : BV]} {(bv<= y x)
                              (bv<= (bv63 0) y)
                              (bv<= y (bv63 100))
                              (bv<= (bv63 0) x)
                              (bv<= x (bv63 100))})

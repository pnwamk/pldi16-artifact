...
(define-metafunction RTR-Base
  ...
  [(val-type (bv63 n)) (Result (Refine ([x : BV]) (bv= x (bv63 n))) TT FF)]
  ...
  [(val-type bvnot)
   (Result (Fun ([x : BV])
                -> (Result (Refine ([z : BV]) (bv= z (bvnot x))) TT FF)) TT FF)]
  [(val-type bvbop)
   (Result (Fun ([x : BV] [y : BV])
                -> (Result (Refine ([z : BV]) (bv= z (bvbop x y))) TT FF))
           TT FF)]
  [(val-type bv=)
...
;;****************************************************
;; x*
;; Helper function used in multiplication
;; Multiply two numbers in the GF(2^8) finite field defined 
;; by the polynomial x^8 + x^4 + x^3 + x + 1 = 0
;; https://www.usenix.org/legacy/event/cardis02/
;;         full_papers/valverde/valverde_html/node12.html
;;****************************************************
(: x-time (-> Byte Byte))
(define (x-time num)
  (let ([n (bitwise-and (* 2 num) #xff)])
    (cond
      [(= 0 (bitwise-and num #x80)) n]
